...
export const generateOTP = middy(async (event) => {
    try {
        ...
        const otpResponse = await generalOTPService.createOTP(merchantId, { ...body, phoneNumber });
        if (otpResponse) {
            return response({ data: phoneNumber, message: 'Otp sent successfully' }, 200);
        } else {
